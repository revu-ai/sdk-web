...
| `host`              | `string`                                     | `"https://api.revu.ai"` | Override only when self-hosting the ingest service, running a regional endpoint, pointing at a local dev server, or routing through your own domain for [first-party ingest](./first-party-ingest.md). Must be http or https.                                          |
...
| `autoIdentify`      | `boolean`                                    | `false`                 | Off by default: anonymous visitors are identified by `anonymous_id` alone and `user_id` stays null until you call `identify()`, so a non-null `user_id` always means a real authenticated account. Enable only if you want a per-device auto id assigned before login (it makes anonymous visitors look identified and is generally not recommended).                                                  |
...
- **`marketing`** and **`functional`** - declarative. The SDK does not
  ...
  (`context.consent`) so the server honors the visitor's choices on
