 ...
 * @property {Record<string, unknown>} context  Engine environment, unprefixed: the session-stable + per-event signals the SDK auto-populates (`user_agent`, `ua_brands` / `ua_mobile` / `ua_platform` (User-Agent Client Hints, low-entropy set, Chromium and secure contexts only), `language`, `languages`, `timezone`, `navigation_type`, `screen_*`, `viewport_*`, `online`, `connection_*`, `webdriver` (stamped only when the browser reports automation), `environment`, `sdk_version`, `consent`, `gpc`, `sample_rate`, and attribution `initial_utm_*` / `utm_*` / `gclid` / `fbclid` / `initial_landing_path` / `initial_seen_at`). Describes the environment the event happened in; kept separate from `properties` so it never collides with caller payload (the de-facto context-vs-properties shape).
