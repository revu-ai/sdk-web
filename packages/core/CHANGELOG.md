...
- **SDK build version.** Every event is stamped with `properties.$sdk_version` so the server can correlate behavior with SDK versions when investigating a regression or rolling out a fix. The same string is exposed as `revu.version` for support tickets and console introspection. Source of truth is `package.json`; `src/version.js` is regenerated from the manifest by `scripts/sync-version.js` (wired as the `prebuild` hook) so a release-day bump touches one file. The generated file is committed so the vanilla example runs straight from `src/` in any browser the SDK supports.
...
- **Environment context.** Every event carries `$user_agent`, viewport and screen dimensions, language, timezone, and referrer host stamped at capture time. UTM and click-id attribution (`utm_source`, `utm_medium`, etc., `gclid`, `fbclid`) are derived server-side from the captured `$pageview.properties.url` and written to the visitor's landing columns, so the SDK does not ship parsing dictionaries that drift.
